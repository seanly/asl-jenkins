...
RUN jenkins-plugin-cli --verbose --plugins rebuild ws-cleanup pipeline-utility-steps blueocean ant filesystem_scm:2.1
COPY --chown=jenkins:jenkins ./asl.hpi /usr/share/jenkins/ref/plugins/

COPY --from=apl /opt/asl-pipeline-library /var/jenkins_home/pipeline-libs/apl
